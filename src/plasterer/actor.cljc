(ns plasterer.actor
  "PlastererActor — the ISCO-08 7123 plastering job-site
  ...
  housebuilder.actor (same wave/domain shape).
  ...
  The unconditional invariant: the Plasterer Advisor can never
  directly commit an operating record the PlastererGovernor refuses,
  and can never make this actor finalize a plastering-execution
  decision (e.g. a specific wall/ceiling finishing step) or override a
  site safety officer's judgment — every commit-record! call is gated
  ...
  SCHEDULING/LOGISTICS ONLY — it never performs plastering work or
  authorizes plastering execution itself."
...
(defn build-graph
  "Build a compiled PlastererActor graph. `store` implements
  `plasterer.store/Store`. `advisor` implements
  `plasterer.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:plasterer-id (:plasterer-id request)
                                    :op (:op proposal)
